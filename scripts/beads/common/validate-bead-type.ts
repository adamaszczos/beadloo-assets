...
export function buildBeadDirectoryIndex(beadDir: string): BeadDirectoryIndex {
  ...
  return {
    metadataIds: Array.from(metadataIds).sort(),
    originalImageIds: Array.from(originalImageIds).sort(),
    thumbnailImageIds: Array.from(thumbnailImageIds).sort(),
...
function validateImageMetadataSync(
  ...
  const index = buildBeadDirectoryIndex(beadDir);
  const metadataIdSet = new Set(index.metadataIds);
  const assetIdSet = new Set(index.assetIds);

  // Check for missing metadata
  for (const id of index.assetIds) {
    if (!metadataIdSet.has(id)) {
      issues.push({
        level: 'warning',
        category: 'metadata',
        message: `Missing metadata file for bead asset: ${id}`
      });
    }
  }

  // Check for orphaned metadata
  for (const id of index.metadataIds) {
    if (!assetIdSet.has(id)) {
      issues.push({
        level: 'warning',
        category: 'metadata',
        message: `Metadata exists but no image assets were found: ${id}.metadata.json`,
        details: 'Color extraction will fall back to a deterministic color until an image is added'
      });
    }
  }

  return {
    images: index.assetIds.length,
    metadata: index.metadataIds.length
  };
}
...
function validateColorCoverage(
  ...
  const jsonIds = Object.keys(colorData.colorMappings).sort();
  const jsonIdSet = new Set(jsonIds);

  for (const id of Array.from(expectedIds).sort()) {
    if (!jsonIdSet.has(id)) {
      issues.push({
        level: 'warning',
        category: 'color-coverage',
        message: `Known bead ID missing from color JSON: ${id}`,
        details: 'Regenerate color data so every metadata-backed bead ID resolves to a HEX color'
      });
    }
  }
  ...
  for (const id of jsonIds) {
    if (!expectedIds.has(id)) {
      issues.push({
        level: 'warning',
        category: 'color-coverage',
        message: `Color JSON has entry for unknown bead ID: ${id}`,
        details: 'Entry is not backed by metadata or local bead assets'
      });
    }
  }
}
...
export function validateBeadType(
  ...
  for (const size of sizes) {
    ...
    if (colorData) {
      ...
      // Check 5: Metadata/assets are fully covered by color mappings
      validateColorCoverage(beadDir, colorData, issues);
    }
  }
